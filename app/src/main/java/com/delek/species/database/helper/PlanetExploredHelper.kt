package com.delek.species.database.helper

class PlanetExploredHelper {

    companion object{
        const val TABLE_NAME: String = "planet_explored"
        const val COLUMN_ID: String = "id"
        const val COLUMN_SPECIE_ID: String = "specie_id"
        const val COLUMN_PLANET_ID: String = "planet_id"
        const val COLUMN_EXPLORED: String = "explored"
        const val COLUMN_COLONY: String = "colony"

        val SQL_CREATE_ENTRIES = buildString {
            append("CREATE TABLE $TABLE_NAME (")
            append("$COLUMN_ID INTEGER PRIMARY KEY AUTOINCREMENT,")
            append("$COLUMN_SPECIE_ID INTEGER,")
            append("$COLUMN_PLANET_ID INTEGER,")
            append("$COLUMN_EXPLORED INTEGER,")
            append("$COLUMN_COLONY INTEGER)")
        }

        const val SQL_DELETE_ENTRIES: String = "DROP TABLE IF EXISTS $TABLE_NAME"
    }

}